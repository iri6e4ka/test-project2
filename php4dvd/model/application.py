...
class Application(object):
    def __init__(self, driver, base_url):
        ...
        self.film_add_page = AddFilmPage(driver,base_url)
    ...
    def add_film(self,film):
        ...
        afp.film_form.title_field.send_keys(film.title)
        afp.film_form.year_field.send_keys(film.year)
        afp.film_form.submit_button.click()

    def film_added(self):
        driver = self.driver
        try:
            self.wait.until(presence_of_element_located((By.CSS_SELECTOR, "title")))
        return True
    ...
    def show_error(self):
        driver = self.driver
        try:
            self.wait.until(presence_of_element_located((By.XPATH, ".//*[@id='updateform']/table/tbody/tr[4]/td[2]/label")))
        return True
        except WebDriverException:
        return False
    ...
    def search(self,user):
        ...
        ip.film_form.search_field.send_keys(user.keyword)

    def not_found(self):
        driver = self.driver
        try:
            self.wait.until(presence_of_element_located((By.CSS_SELECTOR, "content")))
        return True
        except WebDriverException:
        return False

    def something_found(self):
        driver = self.driver
        try:
            self.wait.until(presence_of_element_located((By.CSS_SELECTOR, "title")))
        return True
